        ...
        private static void Game_OnTick(EventArgs args)
        {
            ...
            switch (Game.MapId)
            {
                case GameMapId.SummonersRift:
                    ...
                    foreach (
                        var place in
                            shroomTables.SummonersRift.Where(
                                pos => pos.Distance(ObjectManager.Player.Position) <= Teemo.R.Range && !IsShroomed(pos))
                                .Where(place => Environment.TickCount - lastR > 5000))
                    {
                        Teemo.R.Cast(place);
                    }
                    break;
                case GameMapId.HowlingAbyss:
                    ...
                    foreach (
                        var place in
                            shroomTables.HowlingAbyss.Where(
                                pos => pos.Distance(ObjectManager.Player.Position) <= Teemo.R.Range && !IsShroomed(pos))
                                .Where(place => Environment.TickCount - lastR > 5000))
                    {
                        Teemo.R.Cast(place);
                ...
                case GameMapId.CrystalScar:
                    ...
                    foreach (
                        var place in
                            shroomTables.CrystalScar.Where(
                                pos => pos.Distance(ObjectManager.Player.Position) <= Teemo.R.Range && !IsShroomed(pos))
                                .Where(place => Environment.TickCount - lastR > 5000))
                    {
                        Teemo.R.Cast(place);
                ...
                case GameMapId.TwistedTreeline:
                    ...
                    foreach (
                        var place in
                            shroomTables.TwistedTreeline.Where(
                                pos => pos.Distance(ObjectManager.Player.Position) <= Teemo.R.Range && !IsShroomed(pos))
                                .Where(place => Environment.TickCount - lastR > 5000))
                    {
                        Teemo.R.Cast(place);
